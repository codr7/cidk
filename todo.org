* speed up pairs
** convert Val to use variant
*** add as<> methods and grep "\.as_"
** add list<pair<Val, Val>> cx.pairs
** change Pair to store iterator in constructor and remove in destructor
** remove pool/Ref 
* --
* add try op
** try (foo ()() {}) {...};
